...
current_run <- "mgdrive/two_loci_LarPopNull"
...
for (i in 1:nrow(data)) {
  ...
  print(paste("Running: Threshold =", threshold, "rM =", rM_value))
   ...
   netPar$AdPopRatio_F <- matrix(c(1), nrow = 1, dimnames = list(NULL, c("ZW")))    netPar$AdPopRatio_M <- matrix(c(1-data$introduction_thresholds[[i]], data$introduction_thresholds[[i]]), nrow = 1, dimnames = list(NULL, c("ZZ", "MaMb")))
