...
current_run <- "mgdrive/actual_frequency"
...
# Define Releases
####################
releases <- seq(0,1, .1)  # Generates: 50, 100, 150, ..., 500
...
netPar$AdPopRatio_F <- matrix(c(1-releases[[i]], releases[[i]]), nrow = 1, dimnames = list(NULL, c("ZW", "MW")))

netPar$AdPopRatio_M <- matrix(c(1-releases[[i]], releases[[i]]), nrow = 1, dimnames = list(NULL, c("ZZ", "MM")))
